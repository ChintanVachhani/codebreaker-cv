...
class PuzzleDetection:
    ...
    @staticmethod
    def __findSudokuGrid(image):
        """
        :param image: input image containing puzzle in opencv format
        :return: bounding box (top-left-x, top-left-y, width, height) and the area (a) for the puzzle grid
        """
        # pre-process the image to find puzzle grid
        imageGray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        imageGrayBlurred = cv2.GaussianBlur(imageGray, (11, 11), 0)
        imageGrayBlurredThreshold = cv2.adaptiveThreshold(imageGrayBlurred, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                                          cv2.THRESH_BINARY, 5, 2)
        # invert the image
        imageGrayBlurredThresholdInverted = cv2.bitwise_not(imageGrayBlurredThreshold)
        ...
        biggestSquare = None
        maxSquareArea = 0
        minSquareArea = puzzleSquare.size / 3
    ...
    @staticmethod
    def __findSudokuDigit(image):
        """
        :param image: input image containing digit in opencv format
        :return: boolean if digit found, and bounding box (top-left-x, top-left-y, width, height) of the digit
        """
        # pre-process the image to find puzzle grid
        imageGray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        imageGrayBlurred = cv2.GaussianBlur(imageGray, (11, 11), 0)
